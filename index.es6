//MEDULLA NODE SERVER
let configFile  = 'medulla.config.json';

process.argv.slice(2).forEach((val, index)=>{
	if (index === 0) configFile  = val;
});
...
//GET CUSTOM SETTINGS
try {
	let customSettings = JSON.parse(fs.readFileSync(configFile, 'utf8'));
...
if (cluster.isMaster) {
	...
	const restartServer = after=>{
		if (ordersToStart === handlersModify.length) {
			onRestartEnd.push(after);
	...
	let mm;
	try {mm = require(settings.serverApp)} catch(err) {
